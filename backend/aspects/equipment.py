...
class Equipment(Aspect):
    ...
    def equip(self, item_uuid: str = "") -> dict:
        ...
        if level_required:
            return {"type": "error", "message": "Level requirements are not yet implemented for equipment."}
        ...
        return {"type": "equip_confirm", "item": item.name, "slot": slot, "stat_changes": stat_changes, "message": msg}
    ...
    def unequip(self, slot: str = "") -> dict:
        ...
        return {"type": "unequip_confirm", "item": item_name, "slot": slot, "stat_changes": stat_changes, "message": f"You remove the {item_name} from your {_slot_label(slot).lower()}."}
    ...
    def degrade_durability(self, slot: str, amount: int = 1) -> dict:
        ...
        if durability <= 0:
            return {"status": "broken", "slot": slot, "item_uuid": item_uuid, "message": f"Your {item.name} breaks! Its bonuses are reduced."}

        return {"status": "degraded", "slot": slot, "item_uuid": item_uuid, "durability": durability}
