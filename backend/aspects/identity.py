...
class Identity(Aspect):
    ...
    def name(self, display_name: str = "") -> dict:
        ...
        if len(display_name) < MIN_NAME_LENGTH:
            return {
                "type": "error",
                "message": f"Name must be at least {MIN_NAME_LENGTH} character.",
            }
    ...
    def profile(self) -> dict:
        ...
        # Format timestamps
        created_str = (
            time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(int(created_at) if created_at else 0))
            if created_at
            else "Not set"
        )
        updated_str = (
            time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(int(updated_at) if updated_at else 0))
            if updated_at
            else "Never"
        )
    ...
    def on_equipment_change(self, equipment_summary: str = "", equipped_items: list = None) -> dict:
        ...
        self.data["equipment_summary"] = (
            equipment_summary[:MAX_EQUIPMENT_SUMMARY_LENGTH] if equipment_summary else ""
        )
