...
Dialogue trees are the primary way players experience the game's story, lore, and quest content. Without structured dialogue, NPCs are one-line greeting machines -- they say "Halt, traveler" and then stand there. With dialogue trees, a guard becomes a character who warns about goblin caves to the north, offers a supply-gathering quest, and reacts differently depending on whether the player has completed previous tasks. A merchant becomes someone who shares local gossip alongside their trade inventory. The world stops being a collection of rooms with scenery and starts being a place populated by characters with knowledge and agency.

This system also provides the highest player-facing content per unit of engineering effort of any design in this series. Every dialogue tree added to the registry creates minutes of interactive content that players can discover, explore, and replay. Unlike combat (which requires balancing), building (which requires world modification), or dungeons (which require entity generation), dialogue trees are pure data. A game designer can create hours of NPC conversation content by writing JSON without touching any code. The condition system (has_item, reputation_min, quest_objectives_met) means that the same NPC can present different conversations to different players, creating a sense of a responsive world from static data.

This is the best-fitting design for the current architecture because it requires no new Lambda, no new DynamoDB tables, no new infrastructure patterns, and minimal changes to existing code. The `talk` and `respond` commands run within the existing NPC Lambda, dialogue state is stored on the existing NPC aspect record, and condition evaluation reads from aspects that already exist. It extends the system naturally rather than fighting against it.

## Critical Analysis

**No new Lambda needed -- this is the most architecturally clean design.** Unlike every other design in this series, Dialogue Trees does not require a new Lambda function, a new SNS filter policy, a new CloudWatch rule, or a new DynamoDB table. The `talk` and `respond` commands are player commands on the NPC aspect, processed by the existing NPC Lambda. The dialogue engine is a utility module imported by the NPC aspect. The serverless.yml modification section explicitly states "No new Lambda needed -- dialogue runs within NPC Lambda." This makes it the cheapest to deploy, the simplest to operate, and the easiest to roll back.

**Conversation state stored per-player on NPC data can hit the 400KB item limit.** The NPC aspect stores `conversation_state` as a dict keyed by player UUID. Each entry tracks the current node ID and (per the NPC memory section) the last 10 interactions. With short node IDs and bounded interaction history, each player's state is roughly 200-500 bytes. At 500 bytes per player, 1000 unique players visiting a single NPC produces 500KB of conversation state -- exceeding DynamoDB's 400KB item size limit. This is unlikely for most NPCs (a guard in a remote location may see 10 players total), but a merchant NPC at a major settlement crossroads could realistically accumulate hundreds of unique visitors. The mitigation is to prune conversation state aggressively (evict players who have not interacted in N hours), but this is not described in the design.

**Choice IDs are positional integers, which is fragile.** The `respond` command takes a `choice_id` which is an integer index into the current node's choices array. If the dialogue tree is updated (a new choice inserted at position 2), a player who was mid-conversation with a saved node state could send `respond 3` expecting the old choice 3 but getting the new choice 3 (which was previously choice 4). Using stable string IDs per choice (e.g., "ask_about_quest", "leave") instead of positional integers would prevent this issue. The current design is fine for static trees but breaks silently if trees are ever updated while players are mid-conversation.

**Condition evaluation requires cross-aspect DynamoDB reads.** Each conditional choice in a dialogue node requires loading one of the player's aspects to evaluate. A `reputation_min` condition loads the Faction aspect (1 read). A `has_item` condition loads the Inventory aspect and queries contents (1 GSI query + N reads). A `has_active_quest` condition loads the Quest aspect (1 read). A node with 4 conditional choices from different aspects requires 4 separate DynamoDB reads just to determine which choices to display. This happens on every `talk` and `respond` command. For most nodes with 1-2 conditions this is acceptable, but a complex dialogue hub node with many conditional branches could become expensive. There is no caching of aspect data within a single Lambda invocation.

**LLM fallback implementation path is vague.** The design mentions LLM-enabled NPCs that can respond to freeform player input, but the implementation details are thin. The Suggestion aspect has LLM integration for room descriptions, not NPC dialogue. NPC dialogue would need a different prompt template (including NPC personality, knowledge, conversation history, and location context), different rate limiting (per-player-per-NPC rather than per-room), and different output formatting (in-character speech rather than room description prose). The "call Suggestion aspect's LLM integration" is not a real implementation path -- it would need a new LLM prompt pipeline. The LLM-to-LLM conversation concern (AI agent talking to LLM NPC creating unbounded loops) is noted but the proposed mitigation (3 responses per player per tick) is not implemented anywhere.

**Trade mechanics are completely undefined.** The `show_trade_inventory` action lists items with prices, but the design does not define: what currency is used, how a player actually executes a purchase (what command?), how the price is deducted from the player, whether there is a `buy` command, or how the NPC's inventory is restocked. The `show_player_sellable_items` action similarly has no corresponding `sell` command implementation. This is a critical gap because merchant NPCs are one of the two primary dialogue tree types shown in the registry. A player who navigates the merchant dialogue tree reaches "Here's what I have today:" and then... has no way to actually buy anything. The design punts on this entirely with the open question "What currency do merchants use?" but trade is presented as a core feature of the dialogue system.

**This is the BEST design of all the design documents.** No new Lambda, no new tables, minimal DynamoDB impact (reads only, no writes except conversation state updates), extends existing NPC behavior naturally, provides the most player-facing content per engineering effort, and has the smallest blast radius if something goes wrong (a broken dialogue tree results in a fallback greeting, not corrupted world state). The main risks (item limit on popular NPCs, fragile choice IDs) are both solvable with straightforward mitigations. Every other design in this series should be implemented after this one.
